...
public class BehaviourTreeBuilder<T>
{
	...
	/// </summary>
	private readonly Stack<IParentBehaviourTreeNode<T>> parentNodeStack = new();

	/// <summary>
	///     Last node created.
	/// </summary>
	private IBehaviourTreeNode<T> curNode;

	/// <summary>
	///     Create an action node.
	/// </summary>
	public BehaviourTreeBuilder<T> Do( string name, Func<T, BehaviourTreeStatus> fn,
		Action<T> beforeFn = null, Action<T> afterFn = null )
	{
		...
		var actionNode = new ActionNode<T>( name, fn, beforeFn, afterFn );
...
public class BehaviourTreeBuilder : BehaviourTreeBuilder<ValueType> {}
