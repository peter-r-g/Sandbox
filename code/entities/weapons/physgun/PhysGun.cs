...
[Library( Constants.Weapon.PhysGun )]
public partial class PhysGun : Carriable
{
	...
	public override void Simulate( Client client )
	{
		...
		var grabEnabled = grabbing && Input.Down( InputButton.Attack1 );
		var wantsToFreeze = Input.Pressed( InputButton.Attack2 );
	...
	private static bool IsBodyGrabbed( PhysicsBody body )
	{
		...
		if ( All.OfType<PhysGun>().Any( x => x?.HeldBody?.PhysicsGroup == body?.PhysicsGroup ) )
		{
			return true;
		}

		if ( All.OfType<GravGun>().Any( x => x?.HeldBody?.PhysicsGroup == body?.PhysicsGroup ) )
		{
			return true;
		}
	...
	private void TryUnfreezeAll( Player owner, Vector3 eyePos, Rotation eyeRot, Vector3 eyeDir )
	{
		...
		for ( var i = 0; i < physicsGroup.BodyCount; ++i )
		{
	...
	private void TryStartGrab( Player owner, Vector3 eyePos, Rotation eyeRot, Vector3 eyeDir )
	{
		...
		//
		if ( body.BodyType == PhysicsBodyType.Keyframed && rootEnt is not Player && rootEnt is not CitizenAi )
		{
			return;
		}
	...
	private void UpdateGrab( Vector3 eyePos, Rotation eyeRot, Vector3 eyeDir, bool wantsToFreeze )
	{
		...
		var rotating = Input.Down( InputButton.Use );
		var snapping = false;
	...
	private void Activate()
	{
		...
		if ( !holdBody.IsValid() )
		{
			holdBody = new PhysicsBody( Map.Physics ) {BodyType = PhysicsBodyType.Keyframed};
		}

		if ( !velBody.IsValid() )
		{
			velBody = new PhysicsBody( Map.Physics ) {BodyType = PhysicsBodyType.Dynamic, AutoSleep = false};
		}
	}
