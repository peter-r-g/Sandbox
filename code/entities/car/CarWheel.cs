...
internal struct CarWheel
{
	...
	public bool Raycast( float length, bool doPhysics, Vector3 offset, ref float wheel, float dt )
	{
		...
		if ( !doPhysics && CarEntity.CarDebug )
		{
			...
			if ( tr.Hit )
			{
				DebugOverlay.Circle( wheelPosition, rotation * Rotation.FromYaw( 90 ), wheelRadius,
					Color.Red.WithAlpha( 0.5f ), false );
				DebugOverlay.Line( tr.StartPosition, tr.EndPosition, Color.Red, 0, false );
			}
			else
			{
				DebugOverlay.Circle( wheelPosition, rotation * Rotation.FromYaw( 90 ), wheelRadius,
					Color.Green.WithAlpha( 0.5f ), false );
		...
		var speedDot = MathF.Abs( speed ) > 0.0f
			? MathF.Abs( MathF.Min( Vector3.Dot( velocity, rotation.Up.Normal ) / speed, 0.0f ) )
			: 0.0f;
