...
public class CarCamera : CameraMode
{
	...
	public override void Update()
	{
		...
		carPitch = carPitch.LerpTo(
			car.Grounded ? carRot.Pitch().Clamp( MinCarPitch, MaxCarPitch ) * (speed < 0.0f ? -1.0f : 1.0f) : 0.0f,
			Time.Delta * CarPitchSmoothingSpeed );
		...
		if ( orbitEnabled )
		{
			...
			orbitPitchRot = Rotation.Slerp( orbitPitchRot, Rotation.From( orbitAngles.pitch + carPitch, 0.0f, 0.0f ),
				slerpAmount );
		}
		else
		{
			...
			else
			{
				...
				var slerpAmount = MaxOrbitReturnSpeed > 0.0f
					? Time.Delta * (speedAbs / MaxOrbitReturnSpeed).Clamp( 0.0f, OrbitReturnSmoothingSpeed )
					: 1.0f;

				orbitYawRot = Rotation.Slerp( orbitYawRot, Rotation.FromYaw( targetYaw ), slerpAmount );
				orbitPitchRot = Rotation.Slerp( orbitPitchRot, Rotation.FromPitch( targetPitch + carPitch ),
					slerpAmount );
			}
		...
		currentFov = MaxFovSpeed > 0.0f
			? currentFov.LerpTo( MinFov.LerpTo( MaxFov, speedAbs / MaxFovSpeed ), Time.Delta * FovSmoothingSpeed )
			: MaxFov;
	...
	private void DoThirdPerson( CarEntity car, PhysicsBody body )
	{
		...
		var startPos = carPos;
		var targetPos = startPos + Rotation.Backward * (OrbitDistance * car.Scale) +
		                Vector3.Up * (OrbitHeight * car.Scale);
	...
	private void ApplyShake( float speed )
	{
		...
		var x = Noise.Perlin( pos, 0, 0 ) * length;
		var y = Noise.Perlin( pos, 5.0f, 0 ) * length;
