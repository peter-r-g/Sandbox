...
[Library( Constants.Entity.Car, Spawnable = true )]
public partial class CarEntity : Prop, IUse
{
	...
	[ConVar.ReplicatedAttribute( Constants.Command.CarDebug )]
	public static bool CarDebug { get; set; } = false;

	[ConVar.ReplicatedAttribute( Constants.Command.CarAcceleration )]
	public static float CarAcceleration { get; set; } = 500.0f;
	...
	public override void Spawn()
	{
		...
		trigger.SetupPhysicsFromModel( PhysicsMotionType.Keyframed );
	}
	...
	public override void ClientSpawn()
	{
...

		{
...

			{
				...
				wheel0.SetParent( chassis_axle_front, "Wheel_Steer_R",
					new Transform( Vector3.Zero, Rotation.From( 0, 180, 0 ) ) );
			}

			{
				wheel1 = new ModelEntity();
				wheel1.SetModel( "entities/modular_vehicle/wheel_a.vmdl" );
				wheel1.SetParent( chassis_axle_front, "Wheel_Steer_L",
					new Transform( Vector3.Zero, Rotation.From( 0, 0, 0 ) ) );
			}

			{
				var chassis_steering = new ModelEntity();
				chassis_steering.SetModel( "entities/modular_vehicle/chassis_steering.vmdl" );
				chassis_steering.SetParent( chassis_axle_front, "Axle_front_Center",
					new Transform( Vector3.Zero, Rotation.From( -90, 180, 0 ) ) );
			}
		}

		{
...

			{
				...
				chassis_transmission.SetParent( chassis_axle_rear, "Axle_Rear_Center",
					new Transform( Vector3.Zero, Rotation.From( -90, 180, 0 ) ) );
			}

			{
				...
				wheel2.SetParent( chassis_axle_rear, "Axle_Rear_Center",
					new Transform( Vector3.Left * (0.7f * 40), Rotation.From( 0, 90, 0 ) ) );
			}

			{
				wheel3 = new ModelEntity();
				wheel3.SetModel( "entities/modular_vehicle/wheel_a.vmdl" );
				wheel3.SetParent( chassis_axle_rear, "Axle_Rear_Center",
					new Transform( Vector3.Right * (0.7f * 40), Rotation.From( 0, -90, 0 ) ) );
			}
		}
	}
	...
	public override void Simulate( Client owner )
	{
		...
		using ( Prediction.Off() )
		{
			...
			currentInput.throttle =
				(Input.Down( InputButton.Forward ) ? 1 : 0) + (Input.Down( InputButton.Back ) ? -1 : 0);
			currentInput.turning =
				(Input.Down( InputButton.Left ) ? 1 : 0) + (Input.Down( InputButton.Right ) ? -1 : 0);
	...
	public void OnPrePhysicsStep()
	{
		...
		if ( backWheelsOnGround )
		{
			...
			var acceleration = speedFactor * (accelerateDirection < 0.0f ? CarAcceleration * 0.5f : CarAcceleration) *
			                   accelerateDirection * dt;
		...
		if ( CarDebug )
		{
			DebugOverlay.Line( body.MassCenter, body.MassCenter + rotation.Forward.Normal * 100, Color.White, 0,
				false );
			DebugOverlay.Line( body.MassCenter, body.MassCenter + v.Normal * 100, Color.Green, 0, false );
		}

		var angle = (rotation.Forward.Normal * MathF.Sign( localVelocity.x )).Normal.Dot( v.Normal )
			.Clamp( 0.0f, 1.0f );
		...
		if ( onGround )
		{
			...
			var turnAmount = frontWheelsOnGround
				? MathF.Sign( localVelocity.x ) * 25.0f *
				  CalculateTurnFactor( TurnDirection, MathF.Abs( localVelocity.x ) ) * dt
				: 0.0f;
		...
		{
			var offset = 50 * Scale;
			var s = selfBody.Position + rotation * selfBody.LocalMassCenter + rotation.Right * airRoll * offset +
			        rotation.Down * (10 * Scale);
			...
			if ( currentInput.roll.Clamp( -1, 1 ) != 0 )
			{
				...
				body.ApplyForceAt( selfBody.MassCenter + rotation.Left * (offset * roll),
					rotation.Down * roll * (roll * (body.Mass * force)) );
			...
			{
				var force = 200.0f;
				body.ApplyForceAt( selfBody.MassCenter + rotation.Forward * (offset * airTilt),
					rotation.Down * airTilt * (airTilt * (body.Mass * force)) );
	...
	private static Vector3 VelocityDamping( Vector3 velocity, Rotation rotation, Vector3 damping, float dt )
	{
		...
		var dampingPow = new Vector3( MathF.Pow( 1.0f - damping.x, dt ), MathF.Pow( 1.0f - damping.y, dt ),
			MathF.Pow( 1.0f - damping.z, dt ) );
	...
	private void RaycastWheels( Rotation rotation, bool doPhysics, out bool frontWheels, out bool backWheels, float dt )
	{
		...
		frontWheels =
			frontLeft.Raycast( length + tiltAmount - leanAmount, doPhysics, frontLeftPos * Scale, ref frontLeftDistance,
				dt ) |
			frontRight.Raycast( length + tiltAmount + leanAmount, doPhysics, frontRightPos * Scale,
				ref frontRightDistance, dt );

		backWheels =
			backLeft.Raycast( length - tiltAmount - leanAmount, doPhysics, backLeftPos * Scale, ref backLeftDistance,
				dt ) |
			backRight.Raycast( length - tiltAmount + leanAmount, doPhysics, backRightPos * Scale, ref backRightDistance,
				dt );
	}
	...
	public void OnFrame()
	{
		...
		var frontOffset = 20.0f - Math.Min( frontLeftDistance, frontRightDistance );
		var backOffset = 20.0f - Math.Min( backLeftDistance, backRightDistance );
